...
function displayOpen($assignee){
	global $displayView; // display settings
	foreach ($assignee->tickets as $singleTicket) {
		if(!strcmp($singleTicket->issueType,"Bug") && (!strcmp($singleTicket->status,"Open") || !strcmp($singleTicket->status,"Awaiting Reply")
			|| !strcmp($singleTicket->status,"Dispute"))){
		//echo "<div class='open_ticket'><a href='" . $singleTicket->link ."' style='".colorCode($singleTicket)."' ";
		...
		echo "</a><br>";
		//echo "</a></div><br>";
		//echo "<br><div class='hidden_priority'>" . $singleTicket->priority ."</div></div><br>";		
		}
	}
}
...
// display all the ticket, parameter is staff
function displayBufferAccepted($assignee){
	global $displayView; // display settings
	foreach ($assignee->tickets as $singleTicket) {
		if(!strcmp($singleTicket->issueType,"Bug") && !strcmp($singleTicket->status,"Accepted") 
...
// display all the ticket, parameter is staff
function displayOnGoingAccepted($assignee){
	global $displayView; // display settings
	foreach ($assignee->tickets as $singleTicket) {
		if(!strcmp($singleTicket->issueType,"Bug") && !strcmp($singleTicket->status,"Accepted") 
			&& !empty($singleTicket->statusWhiteBoard)){
...
// display all the ticket, parameter is staff
function displayForReview($assignee){
	global $displayView; // display settings
	foreach ($assignee->tickets as $singleTicket) {
		if(!strcmp($singleTicket->issueType,"Resolution") && !strcmp($singleTicket->status,"Open") 
			&& empty($singleTicket->statusWhiteBoard)){
		echo "<a href='" . $singleTicket->link ."' style='".colorCode($singleTicket)."' ";
		echo "title='". $singleTicket->title ."' target='_blank'>";
		echo $singleTicket->$displayView;
...
// display all the ticket, parameter is staff
function displayBufferDev($assignee){
	global $displayView; // display settings
	foreach ($assignee->tickets as $singleTicket) {
		if(!strcmp($singleTicket->issueType,"Resolution") && !strcmp($singleTicket->status,"Accepted") 
			&& empty($singleTicket->statusWhiteBoard)){
		echo "<a href='" . $singleTicket->link ."' style='".colorCode($singleTicket)."' ";
		echo "title='". $singleTicket->title ."' target='_blank'>";
		echo $singleTicket->$displayView;
...
// display all the ticket, parameter is staff
function displayOnGoingDev($assignee){
	global $displayView; // display settings
	foreach ($assignee->tickets as $singleTicket) {
		if(!strcmp($singleTicket->issueType,"Resolution") && !strcmp($singleTicket->status,"Accepted")
			&& !empty($singleTicket->statusWhiteBoard)){
...
// display all the ticket, parameter is staff
function displayBufferTest($assignee){
	global $displayView; // display settings	
	foreach ($assignee->tickets as $singleTicket) {
		if(!strcmp($singleTicket->issueType,"Resolution") && !strcmp($singleTicket->status,"Development Complete")
			&& empty($singleTicket->statusWhiteBoard)){
		echo "<a href='" . $singleTicket->link ."' style='".colorCode($singleTicket)."' ";
		echo "title='". $singleTicket->title ."' target='_blank'>";
		echo $singleTicket->$displayView;
...
// display all the ticket, parameter is staff
function displayOnGoingTest($assignee){
	global $displayView; // display settings	
	foreach ($assignee->tickets as $singleTicket) {
		if(!strcmp($singleTicket->issueType,"Resolution") && !strcmp($singleTicket->status,"Development Complete")
			&& !empty($singleTicket->statusWhiteBoard)){
...
// display all the ticket, parameter is staff
function displayTestReview($assignee){
	global $displayView; // display settings	
	foreach ($assignee->tickets as $singleTicket) {
		if(!strcmp($singleTicket->issueType,"Resolution") && !strcmp($singleTicket->statusWhiteBoard,"for review")){
		echo "<a href='" . $singleTicket->link ."' style='".colorCode($singleTicket)."' ";
		echo "title='". $singleTicket->title ."' target='_blank'>";
		echo $singleTicket->$displayView;
		echo "</a><br>";
